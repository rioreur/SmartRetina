...
int main(int argc, char** argv)
{
	...
	if (image != NULL)
	{
		...
		// Create the tab from the image and apply the retina on it
		printf("Creating the matrice from the image\n");
		DonneesImageTab* tabImageOrigin = RGBToTab(image);
		printf("Applying the retina\n");
		// It will be use for all the processing
		DonneesImageTab* tabImage = applyRetina(tabImageOrigin, 5);
		printf(" Creating the image wiewed by the retina\n");
		DonneesImageRGB* imageRetina = tabToRGB(tabImage);
		ecrisBMPRGB_Dans(imageRetina, "0 - retina.bmp");
		...
		for(i = 1; i < idRegionsHough->size; i++) // the first one is the background so we skip it
		{
		    ...
		    if (line != NULL)
		    {
		        ...
		        traceLineOnImage(tabImageOrigin, line, 255, 0, 0);
		        free(line);
		        line = NULL;
		    }
		}
		...
		imageRegion = tabToRGB(tabImageOrigin);
		...
		colorGravityPointRegions(tabImageOrigin, gravityPoints, allIds->size-1); // We need to put them in the color tabRegion
		...
		libereDonneesTab(&tabImageOrigin);
		libereDonneesTab(&tabImage);
		libereDonneesImageRGB(&imageRetina);
